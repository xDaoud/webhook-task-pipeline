...
describe("run action", () => {
  it("if actionType is 'filter' it should run and return", () => {
    ...
    expect(runAction("filter", config, payload)).toEqual({
      name: "John",
      age: 30,
    });
  });
  ...
  it("if actionType is 'transform' it should run and return", () => {
    ...
    expect(runAction("transform", config, payload)).toEqual({
      fullName: "John",
    });
  });
  ...
  it("if actionType is 'enrich' it should run and return", () => {
    ...
    expect(runAction("enrich", config, payload)).toEqual({
      name: "John",
      source: "webhook",
      version: 2,
    });
