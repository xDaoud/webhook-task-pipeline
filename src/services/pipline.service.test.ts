...
const mockSubscribers = [
  { id: 'sub-1', pipelineId: 'pipeline-123', url: 'https://example.com/webhook', createdAt: new Date() },
];
...
describe('createPipeline', () => {
  it('should return a pipeline with subscribers', async () => {
    ...
    vi.spyOn(subscriberRepo, 'insertSubscribers').mockResolvedValue(mockSubscribers);
  ...
  it('should throw if insertPipeline fails', async () => {
    vi.spyOn(pipelineRepo, 'insertPipeline').mockRejectedValue(new Error('DB error'));
  ...
  it('should throw if insertSubscribers fails', async () => {
    vi.spyOn(pipelineRepo, 'insertPipeline').mockResolvedValue(mockPipeline);
    vi.spyOn(subscriberRepo, 'insertSubscribers').mockRejectedValue(new Error('DB error'));
